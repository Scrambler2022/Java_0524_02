...
public class Buyer {
	String id;String pw;String name;String nickName;
	int money = 10000;int bonusPoint = 0;
//	int count = 0; // 구매 횟수
	// 구매목록 담기 위해 cart 배열 10개 생성(10개 초과는 담을 수 없음)
	// Product[] cart = new Product[10];
	// => 위 배열을 아래와 같이 Collection으로 변경하면,
	// 개수 제한 없음 + 값 삭제 가능 + 중간 값 삭제해도 중간 공간 비어있지 않고 채워짐
	// 객체 추가: cart[0] = p; => list.add(p) //=> 여기서 p는 Object o = new Product();
	// 호출. 읽기: cart[0] => arr.get()
...
//		}
//	}
	// 위 구매목록 메서드를 배열대신 Array 사용해서 아래와 같이 변경
	void buyList() {
		System.out.print("구매목록:");
		for (int i = 0; i < list.size(); i++) {
			Product p = (Product) list.get(i);
			// 타입이 Object. 형 변환 해줘야함!
			// 위에서 ArrayList<Product>해주면 형 변환((Product)) 안해도 됨!
			// 윗 줄을 안쓰고 아래에서 p 대신 (Product) list.get(i).~ 바로 쓸 수 있음
			if (i == 0) {
